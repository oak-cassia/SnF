...
- `JoinRoom`, `LeaveRoom`, `StartBattle`, `UseSkill`, `Tick`
- Room이 participant combat snapshot과 생사, enemy HP·attack·threat, boss phase, skill
  cooldown과 request sequence를 소유
...
- **시간은 deadline이다.** cooldown은 tick마다 감소시키지 않고 `next_available_at`으로 둔다.
  `ActorContext::observedAt()`이 turn 시작 시각을 주므로 Room은 clock 없이 남고, Step 1이 tick
  없이 성립한다. tick은 wave와 enemy 진행에만 쓴다
- **tick 안의 순서를 고정한다.** player 행동(PlayerId 오름차순) → damage/death 판정 → enemy
  행동(EnemyId 오름차순) → spawn·phase → 종료 판정 → `TickDigest` 생성. threat 동점도 PlayerId
  오름차순으로, clear 보상이 이미 쓰는 규칙과 같다
- **한 tick은 wire에서도 하나의 관찰 단위다.** 결과를 참가자별 한 프레임(`TickDigest`)으로
  묶는다. 이벤트마다 프레임을 쏘면 `max_slots_per_connection`(64)이 0.6초 만에 마르고, 같은
  tick의 damage와 death가 client에서 쪼개져 보인다
...
- **죽음과 퇴장은 다른 상태다.** dead는 여전히 participant다 — `UseSkill`과 heal 대상에서는
  빠지지만 clear 보상은 받는다. leave와 disconnect는 participant에서 제거되고 보상을 포기한다.
  "clear는 Room이 아직 들고 있는 participant에게 지급한다"는 기존 계약이 그대로 성립한다
