...
원본이 아니라 현재 authoritative Player 값을 적용한다. 7.3b의 단일 projector는 durable
...
tail을 반영한다. 7.3c의 schema v4는 checkpoint entry를 새 generation에 작성하고 stream cursor와 meta
pointer만 원자 교체해 큰 snapshot 작성과 award cursor lock을 분리한다. 여러 프로세스 projector의
leader election과 season archive/prune은 아직 범위 밖이다.
