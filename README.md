...
다수의 TCP 연결을 non-blocking으로 처리하면서 연결 생성·종료와 Session 상태를 일관되게
관리해야 합니다.

> Reactor: 한 스레드가 네트워크 이벤트를 받아 `accept`·`recv`·`send`를 실행하는 구조입니다.

#### 결정과 이유

현재 서버는 하나의 Reactor가 Session을 소유하고, `epoll`이 알려준 준비 상태에 따라 입출력을
직접 수행합니다. 따라서 Session과 연결 상태를 한 스레드에서 연속적으로 관리할 수 있습니다.

완료 통지 기반 I/O로 전환할 명확한 요구가 없었기 때문에 현재 범위에서는 epoll Reactor를
유지했습니다.
...
TCP에는 메시지 경계가 없습니다. 한 번의 `recv()`에서 프레임 일부만 받거나 여러 프레임을 한꺼번에
받을 수 있습니다. 프레임 헤더에 본문 길이를 포함하고, 본문 크기는 최대 64 KiB로 제한했습니다.
모든 정수는 big-endian으로 기록합니다.

`FrameDecoder`는 완성되지 않은 입력과 잘못된 입력을 구분하고, 누적된 데이터에서 완성된 프레임을
순서대로 꺼냅니다. `send()`가 일부만 전송한 경우에는 Session이 전송한 위치를 저장하고, 다음
`EPOLLOUT` 이벤트에서 남은 데이터만 이어서 전송합니다.

> 코드: [`src/net/session.cpp`](src/net/session.cpp),
> [`src/protocol/frame_codec.cpp`](src/protocol/frame_codec.cpp)

`request_id`는 요청과 응답을 짝짓습니다. `request_id = 0`은 `BattleDigest`, `BattleCleared`,
`ReturnedToZone`처럼 요청 없이 서버가 보내는 프레임으로 예약했습니다.

#### 파일 디스크립터 재사용 방지

TCP 연결이 닫힌 뒤 같은 파일 디스크립터가 새 연결에 재사용될 수 있습니다. 이전 연결에서
늦게 도착한 Actor 결과를 파일 디스크립터만으로 판정하면 새 클라이언트에게 잘못 전달할 수 있습니다.
...
연결을 수락할 때마다 `generation`을 증가시키고 송신, Session 조회, 명령 전달에서 파일
디스크립터와 `generation`이 모두 일치할 때만 결과를 적용합니다. `epoll` 이벤트 식별자에도
`generation`을 사용하므로 이전 연결의 늦은 이벤트를 현재 Session으로 오인하지 않습니다.

#### 검증

- 프레임을 여러 번에 나누어 수신하거나 여러 프레임을 한 번에 수신하는 경우를 검증합니다.
- 일부만 송신된 경우 다음 송신에서 남은 위치부터 이어지는지 검증합니다.
- 수신 중 연결 종료, `EINTR`, `EAGAIN`, 잘못된 프로토콜 입력을 검증합니다.
- 늦게 도착한 송신 작업과 Actor 비활성화가 재사용된 연결을 변경하지 않는지 검증합니다.
...
- 송신 대기열에 상한을 두지 않습니다.
- 전체 송신 대기열에만 상한을 둡니다.
- 전체·연결별 송신 작업 개수와 Session별 미전송 바이트에 각각 상한을 둡니다.
...
4명이 참가한 Room에서 한 클라이언트만 데이터 수신을 중단했습니다. 포화를 결정적으로 재현하기
위해 테스트에서는 Session의 미전송 데이터를 32 KiB, 서버 송신 버퍼를 8 KiB, 느린 클라이언트의
수신 버퍼를 1 KiB로 낮췄습니다.

상한에 도달한 연결 하나만 종료됐습니다. 나머지 3명은 `BattleDigest`의 순서 번호를 건너뛰지 않았고,
`ParticipantLeft`를 받은 뒤에도 보스를 처치해 전투를 클리어했습니다.
...
- 공유 객체의 변경 지점마다 mutex를 사용합니다.
- 게임 객체마다 FIFO 명령 대기열을 두고 한 번에 하나의 명령만 실행합니다.
...
| 설계 주장 | 대표 검증 |
| --- | --- |
| TCP 부분 입출력 | 분할·병합 수신, 부분 송신 위치, 수신 중 연결 종료 |
| 연결 generation | 이전 연결의 송신·비활성화가 새 Session을 변경하지 않음 |
...
| 느린 클라이언트 | 해당 연결만 종료하고 나머지 참가자는 전투 클리어 |
